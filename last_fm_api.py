...
def search_page_range(query, start_page=1, end_page=2):
    """Fetch track search results from specific page range"""
    collected = []
    ...
    print(f"✅ Collected {len(collected)} tracks from pages {start_page} to {end_page}")
    return collected
...
if __name__ == "__main__":
    # Change these for DAG control
    query = "비"
    start_page = 2
    end_page = 2

    tracks = search_page_range(query, start_page, end_page)
